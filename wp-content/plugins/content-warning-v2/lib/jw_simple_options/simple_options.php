...
/**********************************************************************************
			...
			WWW:			https://github.com/JayWood/jw_simple_options
	...
	public function render_option_field($key, $data){
		switch($data['type']){
			...
			case 'password':
				...
				break;
			case 'number':
				$output = '<input type="number" name="'.$this->prefix.$key.'" id="'.$this->prefix.$key.'" value="'.get_option($this->prefix.$key, $data['def']).'" step="'.$data['step'].'"/>';
				break;
			case 'data_array':
